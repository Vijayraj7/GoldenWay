...
<html
    ...
    <body>
        ...
        // var usersData = {!! json_encode(DB::table('customer_lots')->select('id', 'created_at', 'updated_at', 'pname', 'pamount', 'csId', 'cmpId')->get()) !!};
        var usersData = {!! json_encode(DB::table('customer_transactions')->select('id', 'created_at', 'updated_at', 'tType', 'tAmount', 'wStatus', 'lottId', 'wthId')->get()) !!};
...
<?php
if (isset($_GET['plnid'])) {
    $plans = DB::table('customer_lots')->where('id', $_GET['plnid'])->orWhere('cmpId',$_GET['plnid'])->get();
} elseif (isset($_GET['typ'])) {
    $plans = DB::table('customer_lots')->where('pstatus', '0')->get();
} elseif (isset($_GET['day'])) {
    if(isset($_GET['dayto']) && (strlen($_GET['dayto']) > 2) ){
        $fromDate = $_GET['day'] ?? null;
        $toDate = $_GET['dayto'] ?? null;
        $plans = DB::table('customer_lots')->whereBetween('created_at', [$fromDate, $toDate])->get();
    }else{
        $plans = DB::table('customer_lots')->where('created_at', 'like', $_GET['day'] . '%')->get();
    }
} else {
    $plans = DB::table('customer_lots')->whereNot('pstatus','3')->get();
}
?>
        ...
        date(
            ...
            strtotime($plan->created_at)
                                                ...
                                                @php
                                                    ...
                                                    {
                                                        $pcnt1 = DB::table('customer_lots')->where('csId',$plan->csId)->where('pstatus','1')->where('pname','normal')->count();
                                                        $pcnt2 = DB::table('customer_lots')->where('csId',$plan->csId)->where('pstatus','1')->where('pname','compound')->count();
                                                        $pcnt = $pcnt1 + $pcnt2;
                                                        if($pcnt > 0){
                                                            $isnew = false;
                                                        } else {
                                                ...
                                                <td>
                                                    <a style="color: black;" href="/admin/lott/history?plnid={{$plan->id}}">
                                                    ...
                                                    <br>
                                                        @php
                                                        $cplan = DB::table('customer_lots')->where('id',$plan->cmpId)->first();
                                                        @endphp
                                                    <a style="color: blue;" href="/admin/lott/history?plnid={{$cplan->id}}">
                                                ...
                                                {{-- <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <a
                                                            ...
                                                            href="/admin/lott/history/{{$plan->id}}"><i
                                    ...
                                    $totalcredit += (DB::table('customer_transfers')->where('lottId', $kplan->id)->whereNot('tType','creditadd')->get()->sum('tAmount')*-1);
                                    $addfundttl += DB::table('customer_transfers')->where('lottId', $kplan->id)->where('tType','creditadd')->get()->sum('tAmount');
                                    $alltrans = DB::table('customer_transactions')->where('lottId', $kplan->id)->get();
                                    $totaltrans += $alltrans->sum('tAmount');
                                    // $totaltrans += ($alltrans->sum('tAmount') > $kplan->pamount) ? ($alltrans->count() * $kplan->pamount) : $alltrans->sum('tAmount');
                                    }
                                    ...
                                    // $addfund = (DB::table('customer_transfers')->where('tType', 'creditadd')->where('lottId','creditadd')->get()->sum('tAmount'));
                                ?>
...
@if(isset($_GET['plnid']))
@php
$plnn = DB::table('customer_lots')->where('id',$_GET['plnid'])->first()
@endphp
...
@endif
@include('dashboard.admin.dcards.transactiontable', ['tplans'=>DB::table('customer_transactions')->where('lottId',$_GET['plnid'])
                                ->get()])
@endif
